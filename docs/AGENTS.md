...
1. `docs/architecture.md` — system shape, stack, folder layout
2. `docs/database.md` — schema, keys, procedures/triggers, indexing
3. `docs/api-routes.md` — endpoint contract the frontend depends on
4. `docs/ui-guidelines.md` — component + interaction rules
5. `docs/page-content.md` — copy/labels for each screen (source of truth — don't invent copy)
6. `docs/workload-division.md` — who owns which module, so you don't step on another member's files
...
- **DB**: PostgreSQL 15 (ACID-compliant by default via its WAL — no storage-engine choice needed,
  unlike MySQL)
- **Backend**: FastAPI + `asyncpg`, JWT auth. **No ORM** (SQLAlchemy or any other table-mapping
  ...
  `database.md` §6 directly through an `asyncpg` connection pool. Pydantic models are fine and
...
- **Every appointment write** (create/reschedule) must go through the overlap-check logic —
  ...
  `fn_reschedule_appointment()` (`database.md` §1.2/§7.1/§7.3) — routes call these functions,
...
- **Treatments/consultations can only be attached to a `Completed` appointment.** **Decision:
  ...
  `database.md` §6.6), not a service-layer check — don't add a redundant Python-side check for
